...
void main() {
  ...
  vec3 ambient = k_ambient * texture(diffuse, tc * 5.0).rgb;
  // vec3 ambient = k_ambient * color_test.xyz;
  vec3 color = ambient;

  vec3 pos_camera =
      (view * model * vec4(pos_world.x, pos_world.y, pos_world.z, 1)).xyz;
  pos_camera = pos_world;

  vec4 norm_view =
      view_normal * vec4(norm_world.x, norm_world.y, norm_world.z, 1);
  vec3 n = norm_world.xyz;
  ...
  // out_col = vec4(pv_color.x, pv_color.y, pv_color.z, pv_color.w);
