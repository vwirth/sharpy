...
TextureMipWrapper texture_construct_mip(torch::Tensor tex, int max_mip_level,
  ...
  for (int i = 1; i <= p.mipLevelMax; i++) {
    ...
    NVDR_CHECK_CUDA_ERROR(cudaLaunchKernel(
        build_func_tbl[channel_div_idx], gridSize, blockSize, args, 0, stream));
  }
...
torch::Tensor texture_fwd_mip(torch::Tensor tex, torch::Tensor uv,
  ...
  if (p.enableMip) {
    if (has_mip_stack) {
      TORCH_CHECK(at::cuda::check_device(mip_stack), __func__,
  ...
  if (cube_mode) func_idx += TEX_MODE_COUNT;  // Cube variant.
  ...
  // Launch kernel.
  NVDR_CHECK_CUDA_ERROR(cudaLaunchKernel(func_tbl[func_idx], gridSize,
                                         blockSize, args, 0, stream));
...
texture_grad_linear_mipmap_linear(torch::Tensor tex, torch::Tensor uv,
  ...
  if (p.enableMip && !has_mip_stack) {
    ...
    void* mip_grad_func_tbl[3] = {(void*)MipGradKernel1, (void*)MipGradKernel2,
                                  (void*)MipGradKernel4};
    NVDR_CHECK_CUDA_ERROR(cudaLaunchKernel(mip_grad_func_tbl[channel_div_idx],
                                           gridSize, blockSize, args,
                                           sharedBytes, stream));
  }
...
std::tuple<torch::Tensor, torch::Tensor> texture_grad_linear(
    torch::Tensor tex, torch::Tensor uv, torch::Tensor dy, int filter_mode,
    int boundary_mode) {
  torch::Tensor empty_tensor;
  std::vector<torch::Tensor> empty_vector;
  std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
...
texture_grad_linear_mipmap_nearest(torch::Tensor tex, torch::Tensor uv,
  ...
  std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor,
             std::vector<torch::Tensor> >
      result = texture_grad_linear_mipmap_linear(
          tex, uv, dy, uv_da, mip_level_bias, mip_wrapper, mip_stack,
