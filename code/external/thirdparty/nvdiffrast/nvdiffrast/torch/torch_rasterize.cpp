...
std::tuple<torch::Tensor, torch::Tensor> nvdiffrast::rasterize_fwd_cuda(
  ...
  NVDR_CHECK(pos.get_device() == stateWrapper.cudaDeviceIdx,
             "CudaRaster context must must reside on the same device as input "
             "tensors");
  ...
  if (instance_mode)
    NVDR_CHECK(pos.sizes().size() == 3 && pos.size(0) > 0 && pos.size(1) > 0 &&
                   pos.size(2) == 4,
               "instance mode - pos must have shape [>0, >0, 4]");
  else {
    NVDR_CHECK(pos.sizes().size() == 2 && pos.size(0) > 0 && pos.size(1) == 4,
               "range mode - pos must have shape [>0, 4]");
    NVDR_CHECK(
        ranges.sizes().size() == 2 && ranges.size(0) > 0 && ranges.size(1) == 2,
        "range mode - ranges must have shape [>0, 2]");
  }
  ...
  int depth =
      instance_mode
          ? pos.size(0)
          : ranges.size(0);  // Depth of tensor, not related to depth buffering.
  NVDR_CHECK(height > 0 && width > 0, "resolution must be [>0, >0]");

  // Check resolution compatibility with CudaRaster.
  TORCH_CHECK(height <= CR_MAXVIEWPORT_SIZE && width <= CR_MAXVIEWPORT_SIZE,
              "resolution must be [<=", CR_MAXVIEWPORT_SIZE,
              ", <=", CR_MAXVIEWPORT_SIZE, "]");
  TORCH_CHECK(((height | width) & (CR_TILE_SIZE - 1)) == 0,
              "width and height must be divisible by ", CR_TILE_SIZE);
  ...
  const int32_t* rangesPtr =
      instance_mode ? 0 : ranges.data_ptr<int32_t>();  // This is in CPU memory.
  ...
  cr->setRenderModeFlags(enablePeel
                             ? CR::CudaRaster::RenderModeFlag_EnableDepthPeeling
                             : 0);  // No backface culling.
  if (enablePeel)
    cr->swapDepthAndPeel();  // Use previous depth buffer as peeling depth
                             // input.

  // Run CudaRaster in one large batch. In case of error, the workload could be
  // split into smaller batches - maybe do that in the future.
  cr->deferredClear(0u);
  ...
  NVDR_CHECK(!((uintptr_t)p.out_db & 15),
             "out_db output tensor not aligned to float4");
  ...
  // Launch CUDA kernel.
  void* args[] = {&p};
  NVDR_CHECK_CUDA_ERROR(cudaLaunchKernel((void*)RasterizeCudaFwdShaderKernel,
...
torch::Tensor nvdiffrast::rasterize_grad_db(torch::Tensor pos,
                                            torch::Tensor tri,
  ...
  p.height = out.size(1);
  p.width = out.size(2);
  NVDR_CHECK(p.depth > 0 && p.height > 0 && p.width > 0,
             "resolution must be [>0, >0, >0]");

  // Check other shapes.
  if (p.instance_mode)
    NVDR_CHECK(pos.sizes().size() == 3 && pos.size(0) == p.depth &&
                   pos.size(1) > 0 && pos.size(2) == 4,
               "pos must have shape [depth, >0, 4]");
  else
    NVDR_CHECK(pos.sizes().size() == 2 && pos.size(0) > 0 && pos.size(1) == 4,
               "pos must have shape [>0, 4]");
  NVDR_CHECK(tri.sizes().size() == 2 && tri.size(0) > 0 && tri.size(1) == 3,
             "tri must have shape [>0, 3]");
  NVDR_CHECK(out.sizes().size() == 4 && out.size(0) == p.depth &&
                 out.size(1) == p.height && out.size(2) == p.width &&
                 out.size(3) == 4,
             "out must have shape [depth, height, width, 4]");
  NVDR_CHECK(dy.sizes().size() == 4 && dy.size(0) == p.depth &&
                 dy.size(1) == p.height && dy.size(2) == p.width &&
                 dy.size(3) == 4,
             "dy must have shape [depth, height, width, 4]");
  if (enable_db)
    NVDR_CHECK(ddb.sizes().size() == 4 && ddb.size(0) == p.depth &&
                   ddb.size(1) == p.height && ddb.size(2) == p.width &&
                   ddb.size(3) == 4,
               "ddb must have shape [depth, height, width, 4]");

  // Ensure gradients are contiguous.
  torch::Tensor dy_ = dy.contiguous();
  torch::Tensor ddb_;
  if (enable_db) ddb_ = ddb.contiguous();
  ...
  NVDR_CHECK(!((uintptr_t)p.ddb & 15),
             "ddb input tensor not aligned to float4");

  // Choose launch parameters.
  dim3 blockSize =
      getLaunchBlockSize(RAST_GRAD_MAX_KERNEL_BLOCK_WIDTH,
  ...
  void* args[] = {&p};
  void* func =
      enable_db ? (void*)RasterizeGradKernelDb : (void*)RasterizeGradKernel;
...
torch::Tensor nvdiffrast::rasterize_grad(torch::Tensor pos, torch::Tensor tri,
                                         torch::Tensor out, torch::Tensor dy) {
