...
template <class T>
static __device__ __forceinline__ T zero_value(void);
template <>
__device__ __forceinline__ float zero_value<float>(void) {
  return 0.f;
}
template <>
__device__ __forceinline__ float2 zero_value<float2>(void) {
  return make_float2(0.f, 0.f);
}
template <>
__device__ __forceinline__ float4 zero_value<float4>(void) {
...
template <class T>
static __device__ __forceinline__ void swap(T& a, T& b) {
  T temp = a;
  a = b;
  b = temp;
}
...
#if __CUDA_ARCH__ >= 700  // Warp match instruction __match_any_sync() is only
                          // available on compute capability 7.x and higher
...
#define caAtomicAdd(ptr, value)                 \
  do {                                          \
    if (_ca_leader) *_ca_ptr = 0.f;             \
    atomicAdd(_ca_ptr, (value));                \
    if (_ca_leader) atomicAdd((ptr), *_ca_ptr); \
  } while (0)
