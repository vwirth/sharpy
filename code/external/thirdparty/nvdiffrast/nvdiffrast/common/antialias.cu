...
static __forceinline__ __device__ bool same_sign(float a, float b) {
  return (__float_as_int(a) ^ __float_as_int(b)) >= 0;
}
static __forceinline__ __device__ bool rational_gt(float n0, float n1, float d0,
                                                   float d1) {
  return (n0 * d1 > n1 * d0) == same_sign(d0, d1);
}
static __forceinline__ __device__ int max_idx3(float n0, float n1, float n2,
...
struct AAWorkItem {
  ...
  int px, py;  // Pixel x, y.
  unsigned int
      pz_flags;  // High 16 bits = pixel z, low 16 bits = edge index and flags.
  float alpha;   // Antialiasing alpha value. Zero if no AA.
};

//------------------------------------------------------------------------
// Hash functions. Adapted from public-domain code at
// http://www.burtleburtle.net/bob/hash/doobs.html

#define JENKINS_MAGIC (0x9e3779b9u)
static __device__ __forceinline__ void jenkins_mix(unsigned int& a,
                                                   unsigned int& b,
                                                   unsigned int& c) {
  a -= b;
  a -= c;
...
// Helper class for hash index iteration. Implements simple odd-skip linear
// probing with a key-dependent skip.
class HashIndex {
 public:
  __device__ __forceinline__ HashIndex(const AntialiasKernelParams& p,
                                       uint64_t key) {
  ...
  __device__ __forceinline__ void next(void) {
    m_idx = (m_idx + m_skip) & m_mask;
  }
...
static __device__ __forceinline__ void hash_insert(
    const AntialiasKernelParams& p, uint64_t key, int v) {
  HashIndex idx(p, key);
  while (1) {
    uint64_t prev = atomicCAS((unsigned long long*)&p.evHash[idx.get()], 0,
  ...
  if (a != 0 && a != v) atomicCAS(q + 3, 0, v);
}

static __device__ __forceinline__ int2 hash_find(const AntialiasKernelParams& p,
                                                 uint64_t key) {
  HashIndex idx(p, key);
  while (1) {
    uint4 entry = p.evHash[idx.get()];
    uint64_t k = ((uint64_t)entry.x) | (((uint64_t)entry.y) << 32);
    if (k == key || k == 0) return make_int2((int)entry.z, (int)entry.w);
    idx.next();
  }
}

static __device__ __forceinline__ void evhash_insert_vertex(
    const AntialiasKernelParams& p, int va, int vb, int vn) {
  if (va == vb) return;
...
static __forceinline__ __device__ int evhash_find_vertex(
    const AntialiasKernelParams& p, int va, int vb, int vr) {
  if (va == vb) return -1;
...
__global__ void AntialiasFwdMeshKernel(const AntialiasKernelParams p) {
  ...
  if (idx >= p.numTriangles) return;

  int v0 = p.tri[idx * 3 + 0];
  int v1 = p.tri[idx * 3 + 1];
  int v2 = p.tri[idx * 3 + 2];

  if (v0 < 0 || v0 >= p.numVertices || v1 < 0 || v1 >= p.numVertices ||
...
__global__ void AntialiasFwdDiscontinuityKernel(const AntialiasKernelParams p) {
  ...
  int pz = blockIdx.z;
  if (px >= p.width || py >= p.height || pz >= p.n) return;
  ...
  int pidx1 = pidx0;
  if (px < p.width - 1) pidx1 += 4;
  float tri1 = p.rasterOut[pidx1];

  // Look down, clamp at edge.
  int pidx2 = pidx0;
  if (py < p.height - 1) pidx2 += p.width << 2;
  ...
  if (tri2 != tri0)
    p.workBuffer[idx] =
        make_int4(px, py, (pz << 16) + (1 << AAWorkItem::FLAG_DOWN_BIT), 0);
}
...
__global__ void AntialiasFwdAnalysisKernel(const AntialiasKernelParams p) {
  ...
  for (;;) {
    ...
    if (threadIdx.x == 0)
      s_base =
          atomicAdd(&p.workBuffer[0].y, AA_ANALYSIS_KERNEL_THREADS_PER_BLOCK);
    __syncthreads();
    int thread_idx = s_base + threadIdx.x;
    if (thread_idx >= workCount) return;
    ...
    if (tri0 >= 0 && tri1 >= 0) tri = (zt0.x < zt1.x) ? tri0 : tri1;
    if (tri == tri1) {
    ...
    if (tri < 0 || tri >= p.numTriangles) continue;
    ...
    if (vi0 < 0 || vi0 >= p.numVertices || vi1 < 0 || vi1 >= p.numVertices ||
        vi2 < 0 || vi2 >= p.numVertices)
      continue;

    // Fetch opposite vertex indices. Use vertex itself (always silhouette) if
    // no opposite vertex exists.
    ...
    float bb =
        (x1 - x0) * (y2 - y0) - (x2 - x0) * (y1 - y0);  // Triangle itself.
    ...
    if (same_sign(a0, bb) || same_sign(a1, bb) || same_sign(a2, bb)) {
      ...
      if (d) {
        nvdiffrast::swap(x0, y0);
        nvdiffrast::swap(x1, y1);
        nvdiffrast::swap(x2, y2);
      }
      ...
      if (di == 0 && same_sign(a0, bb) && fabsf(dy0) >= fabsf(dx0))
        dc = d0 / dy0;
      if (di == 1 && same_sign(a1, bb) && fabsf(dy1) >= fabsf(dx1))
        dc = d1 / dy1;
      if (di == 2 && same_sign(a2, bb) && fabsf(dy2) >= fabsf(dx2))
        dc = d2 / dy2;
      ...
      if (dc > -eps && dc < 1.f + eps) {
        ...
        float* pOutput =
            p.output + (alpha > 0.f ? pixel0 : pixel1) * p.channels;
...
__global__ void AntialiasGradKernel(const AntialiasKernelParams p) {
  ...
  for (;;) {
    ...
    if (item.w == 0) continue;  // No effect.
    ...
    for (int i = 0; i < p.channels; i++) {
      ...
      if (dy != 0.f) {
        ...
        // Update color gradients. No coalescing because all have different
        // targets.
        float v = alpha * dy;
    ...
    // Bail out if vertex indices are corrupt.
    bool vtxFail =
        (vi1 < 0 || vi1 >= p.numVertices || vi2 < 0 || vi2 >= p.numVertices);
    ...
    // XY flip for horizontal edges.
    if (d) {
      nvdiffrast::swap(p1.x, p1.y);
      nvdiffrast::swap(p2.x, p2.y);
      nvdiffrast::swap(pxh, pyh);
      nvdiffrast::swap(fx, fy);
    }
    ...
    if (d) {
      nvdiffrast::swap(gp1x, gp1y);
      nvdiffrast::swap(gp2x, gp2y);
    }
