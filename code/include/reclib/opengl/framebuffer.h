...
class _API FramebufferImpl {
 public:
  ...
  static inline std::string type_to_str() { return "FramebufferImpl"; }
  ...
  // attaches a depth buffer (with optional stencil component) to the
  // framebuffer, if no valid texture is given, a default one is created
...
template class _API reclib::opengl::NamedHandle<
    reclib::opengl::FramebufferImpl>;  // needed for Windows DLL export
