...
reclib::opengl::MaterialImpl::MaterialImpl(const std::string& name)
    : name(name) {}
...
reclib::opengl::MaterialImpl::MaterialImpl(const std::string& name,
  ...
  // ambient, diffuse, specular and emissive color are handled via fallback 1x1
  // textures parse assimp material parameters
  ...
  if (mat_ai->GetTextureCount(aiTextureType_DIFFUSE) > 0) {
    ...
    texture_map["diffuse"] = Texture2D(name + "_diffuse_" + name_ai.C_Str(),
                                       base_path / path_ai.C_Str());
  } else if (mat_ai->Get(AI_MATKEY_COLOR_DIFFUSE, vec3_value) == AI_SUCCESS) {
    // 1x1 fallback texture
    texture_map["diffuse"] =
        Texture2D(name + "_diffuse_" + name_ai.C_Str(), 1, 1, GL_RGB32F, GL_RGB,
                  GL_FLOAT, &vec3_value.r);
  }
  // specular
  if (mat_ai->GetTextureCount(aiTextureType_SPECULAR) > 0) {
    aiString path_ai;
    mat_ai->GetTexture(aiTextureType_SPECULAR, 0, &path_ai);
    texture_map["specular"] = Texture2D(name + "_specular_" + name_ai.C_Str(),
                                        base_path / path_ai.C_Str());
  } else if (mat_ai->Get(AI_MATKEY_COLOR_SPECULAR, vec3_value) == AI_SUCCESS) {
    // 1x1 fallback texture
    texture_map["specular"] =
        Texture2D(name + "_specular_" + name_ai.C_Str(), 1, 1, GL_RGB32F,
                  GL_RGB, GL_FLOAT, &vec3_value.r);
  }
  ...
  if (mat_ai->GetTextureCount(aiTextureType_AMBIENT) > 0) {
    ...
    texture_map["ambient"] = Texture2D(name + "_ambient_" + name_ai.C_Str(),
                                       base_path / path_ai.C_Str());
  } else if (mat_ai->Get(AI_MATKEY_COLOR_AMBIENT, vec3_value) == AI_SUCCESS) {
    // 1x1 fallback texture
    texture_map["ambient"] =
        Texture2D(name + "_ambient_" + name_ai.C_Str(), 1, 1, GL_RGB32F, GL_RGB,
                  GL_FLOAT, &vec3_value.r);
  }
  // emissive
  ...
  if (mat_ai->GetTextureCount(aiTextureType_EMISSIVE) > 0) {
    ...
    texture_map["emissive"] = Texture2D(name + "_emissive_" + name_ai.C_Str(),
                                        base_path / path_ai.C_Str());
  } else if (mat_ai->Get(AI_MATKEY_COLOR_EMISSIVE, vec3_value) == AI_SUCCESS) {
    // 1x1 fallback texture
    texture_map["emissive"] =
        Texture2D(name + "_emissive_" + name_ai.C_Str(), 1, 1, GL_RGB32F,
                  GL_RGB, GL_FLOAT, &vec3_value.r);
  }

  // heightmap / normalmap (obj: map_Bump somehow is aiTextureType_HEIGHT, not
  // aiTextureType_NORMALS) TODO test other file formats
  if (mat_ai->GetTextureCount(aiTextureType_HEIGHT) > 0) {
    aiString path_ai;
    mat_ai->GetTexture(aiTextureType_HEIGHT, 0, &path_ai);
    texture_map["normalmap"] = Texture2D(name + "_normal_" + name_ai.C_Str(),
                                         base_path / path_ai.C_Str());
  }

  // alphamap (TODO how to handle alphamap vs opacity parameter, or alpha
  // channel of diffuse texture such as in SMG?)
  if (mat_ai->GetTextureCount(aiTextureType_OPACITY) > 0) {
    aiString path_ai;
    mat_ai->GetTexture(aiTextureType_OPACITY, 0, &path_ai);
    texture_map["alphamap"] = Texture2D(name + "_alpha_" + name_ai.C_Str(),
                                        base_path / path_ai.C_Str());
  }

  // roughness texture (TODO do we want this, or just the static roughness
  // param?)
  if (mat_ai->GetTextureCount(aiTextureType_SHININESS) > 0) {
    aiString path_ai;
    mat_ai->GetTexture(aiTextureType_SHININESS, 0, &path_ai);
    texture_map["roughness"] = Texture2D(name + "_roughness_" + name_ai.C_Str(),
                                         base_path / path_ai.C_Str());
  }
  // displacement map
  if (mat_ai->GetTextureCount(aiTextureType_DISPLACEMENT) > 0) {
    ...
    texture_map["displacement"] = Texture2D(
        name + "_displacement_" + name_ai.C_Str(), base_path / path_ai.C_Str());
  }
  ...
  if (mat_ai->GetTextureCount(aiTextureType_LIGHTMAP) > 0) {
    ...
    texture_map["lightmap"] = Texture2D(name + "_light_" + name_ai.C_Str(),
                                        base_path / path_ai.C_Str());
  }

  // whatever
  if (mat_ai->GetTextureCount(aiTextureType_UNKNOWN) > 0)
    std::cerr << "Warn: material <" << name
              << "> has unknown texture: " << name_ai.C_Str() << std::endl;
}

reclib::opengl::MaterialImpl::~MaterialImpl() {}

void reclib::opengl::MaterialImpl::bind(const Shader& shader) const {
  // bind parameters as uniforms
  for (const auto& entry : int_map) shader->uniform(entry.first, entry.second);
  for (const auto& entry : float_map)
    shader->uniform(entry.first, entry.second);
  for (const auto& entry : vec2_map) shader->uniform(entry.first, entry.second);
  for (const auto& entry : vec3_map) {
    shader->uniform(entry.first, entry.second);
  }
  for (const auto& entry : vec4_map) shader->uniform(entry.first, entry.second);
...
void reclib::opengl::MaterialImpl::unbind() const {
  // unbind textures
  for (const auto& entry : texture_map) entry.second->unbind();
}
